...
vertex_indices = UInt32.(collect(1:4) .- 1)
...
function main()
    ...
    datasize = sizeof(eltype(vertex_indices)) * length(vertex_indices)
    index_buffer = Buffer(device, datasize, BUFFER_USAGE_INDEX_BUFFER_BIT, VK_SHARING_MODE_EXCLUSIVE, [0])
    index_buffer_memreqs = get_buffer_memory_requirements(index_buffer.device, index_buffer)
    index_buffer_memory = DeviceMemory(index_buffer.device, index_buffer_memreqs, MEMORY_PROPERTY_HOST_VISIBLE_BIT | MEMORY_PROPERTY_HOST_COHERENT_BIT)
    bind_buffer_memory(index_buffer.device, index_buffer, index_buffer_memory, 0)
    dataptr = unwrap(map_memory(index_buffer_memory.device, index_buffer_memory, 0, datasize))
    unsafe_copyto!(Ptr{eltype(vertex_indices)}(dataptr), pointer(vertex_indices), length(vertex_indices))
    unwrap(unmap_memory(index_buffer_memory.device, index_buffer_memory))
    ...
    cmd_bind_index_buffer(command_buffer, index_buffer, 0, VK_INDEX_TYPE_UINT32)
    ...
    cmd_begin_render_pass(command_buffer, RenderPassBeginInfo(render_pass, framebuffer, Rect2D(Offset2D(0,0),Extent2D(width, height)), [vk.VkClearValue(vk.VkClearColorValue(tuple(rand(Float16, 3)..., 1f0)))]), VK_SUBPASS_CONTENTS_INLINE)
