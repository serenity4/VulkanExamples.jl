...
function main(output_png)
    ...
    vertices = PosColor.(invert_y_axis.(points), colors)
    ...
    vertex_input_state = PipelineVertexInputStateCreateInfo([VertexInputBindingDescription(eltype(vertices), 0)], VertexInputAttributeDescription(eltype(vertices), 0))
